...
def main(args):
    ...
    model = GPT_models[args.gpt_model](
        ...
        tok_drop_method=args.tok_drop_method,
    ).to(device)
    ...
    if args.gpt_ckpt:
        ...
        del checkpoint["model"]["cls_embedding.embedding_table.weight"]
        ...
        if "/data_hdd/xyliu/llamagen_experiments" in args.gpt_ckpt:
            print("load from old ckpt")
            # train_steps = int(args.gpt_ckpt.split('/')[-1].split('.')[0])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            # print(x.shape, y.shape, z_indices.shape, c_indices.shape)
            ...
            logger.info(f"local_step: {local_step}, steps_per_epoch: {steps_per_epoch}, current_lr: {current_lr}")
            if local_step == steps_per_epoch - 1:
            ...
            if epoch % 100 == 0 and local_step == steps_per_epoch - 1:
                if rank == 0:
                    if not args.no_compile:
            ...
            if epoch == args.epochs - 1 and local_step == steps_per_epoch - 1:
                if rank == 0:
                    if not args.no_compile:
                        model_weight = model.module._orig_mod.state_dict()
                    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt-every", type=int, default=300)
    parser.add_argument("--gradient-accumulation-steps", type=int, default=1)
    parser.add_argument("--mixed-precision", type=str, default='bf16', choices=["none", "fp16", "bf16"]) 
    #
    parser.add_argument("--tok-drop-method", type=str, choices=['static', 'adaptive'], default="static")
